	...
	$.extend(ColResize.prototype, {
		...
		fnRestoreState: function () {
			...
			if (sizeMap.length !== cols.length) {
				this.s.dt.oInstance.oApi._fnLog(
					this.dt,
					1,
					"ColResize: Array size doesn't match number of columns."
				)
				return
			}

			self.s.state.maxTableWidth =
				self._fnGetBodyScroll().length > 0 ? 0 : this._fnGetTable().width()
		...
		_fnConstruct: function () {
			...
			$(document).on('mousemove.ColResize touchmove.ColResize', function (e) {
				if (self.s.state.isDragging) {
					...
					if (self.s.state.maxTableWidth > 0) {
						let currentTableWidth = self.s.state.$element
							.closest('table')
							.width()
						if (currentTableWidth > self.s.state.maxTableWidth) {
							self._fnApplyWidth(
								changedWidth +
									(self.s.state.maxTableWidth - currentTableWidth,
									self.s.state.$element,
									self.s.state.column)
							)
			...
			$(document).on('mouseup.ColResize touchend.ColResize', function () {
				if (self.s.state.isDragging) {
					...
					self.s.opts.onResizeEnd(
						self._fnMapColumn(self.s.state.column),
						mappedColumns
					)
					...
					$columnNode.on(
						...
						function (e) {
							...
							if (self._fnIsInDragArea($node, e)) {
								...
								self.s.state.maxTableWidth =
									self._fnGetBodyScroll().length > 0
										? 0
										: $node.closest('table').width()
								self.s.state.originalTableWidth = $node.closest('table').width()
								self.s.state.originalWidth[$node.index()] =
									self._fnGetCurrentWidth($node)
								...
								self.s.state.isLastColumnDragging =
									self._fnIsLastResizableColumnDragging(column)

								self.s.opts.onResizeStart(
									null,
									self._fnGetAllColumns().map(self._fnMapColumn)
								)
							}
						}
					)
		...
		_fnRemovePercentWidths: function (column, $node) {
			if ($node.attr('style') && $node.attr('style').indexOf('%') !== -1) {
				this.s.dt.oInstance.oApi._fnLog(
					this.dt,
					1,
					'ColResize: column styles in percentages is not supported, trying to convert to px on the fly.'
				)
		...
		_fnApplyWidth: function (changedWidth, element, column) {
			...
			changedWidth = this.s.opts.hasBoundCheck
				? this._fnBoundCheck(changedWidth, element)
				: changedWidth
			...
			let shouldChangeTableWidth =
				element.closest('.dataTables_scroll').length > 0 &&
				$table.width() + changedWidth >
					element.closest('.dataTables_scroll').width()
			if (shouldChangeTableWidth) {
				$table.width(self.s.state.originalTableWidth + changedWidth)
			}

			// possible body table
			let scrollBodyTh = element
			...
			let scrollFooterTh = element
				.closest('.dataTables_scroll')
				.find(
					'.dataTables_scrollFoot table th:nth-child(' +
						(element.index() + 1) +
						')'
				)
			...
			if (element.closest('.dataTables_scroll').length > 0) {
				let additionalStylesForHiddenThRows =
					';padding-top: 0px;padding-bottom: 0px;border-top-width: 0px;border-bottom-width: 0px;height: 0px;'
				this._fnGetAllColumns().forEach(function (column) {
					let $hbTh = $(column.nTh)
					let currentIndex = $hbTh.index()
					let currentStyles =
						$hbTh.attr('style') + additionalStylesForHiddenThRows
		...
		_fnBoundCheck: function (changedWidth, element) {
			let thWishWidth =
				(typeof this.s.state.originalWidth[element.index()] != 'undefined'
					? this.s.state.originalWidth[element.index()]
					: this._fnGetCurrentWidth(element)) + changedWidth
